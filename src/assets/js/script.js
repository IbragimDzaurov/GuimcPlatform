let divs = Array.from(document.querySelectorAll('.sub-video-container')),
        minWidth = 100,
        minHeight = 100,
        maxWidth = 900,
        maxHeight = 900,
...
function mousedown(e) {
    ...
    if (!isResizing && !e.target.classList.contains('offset-container')) {
        window.addEventListener('mousemove', mousemove);
        window.addEventListener('mouseup', mouseup);
    }
...
class Video {
    constructor (mainVideo, subVideoIcons) {
        this.mainVideo = mainVideo;
        this.subVideoIcons = subVideoIcons;
    };

    changeVideo = () => {
        this.subVideoIcons.forEach((subVideoIcon) => {
            subVideoIcon.addEventListener('click', () => {
                const video = subVideoIcon.parentElement.querySelector('video');
        
                const mainVideoSrc = this.mainVideo.src;
...
const $mainVideo = document.querySelector('.video > video'),
    $subVideoIcons = document.querySelectorAll('.sub-video > .icon');

const video = new Video($mainVideo, $subVideoIcons);
video.changeVideo();
